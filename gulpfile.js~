...
gulp.task('sass', function () {
    var bundle = gulp.src('./scss/styles.scss').pipe(sass()).on('error', function (error) {
        console.log(error);
        this.emit('end');
    });

    return bundle.pipe(concat('style.scss')).pipe(gulp.dest('build/css/'));
});
...
gulp.task('js', function () {
    ...
    return bundle.pipe(source('./js/app.js')).pipe(gulp.dest('build/js'));
});
